<template>
  <div id="daily_growth_chart_china"></div>
</template>
<script>
export default {
  data () {
    ...
    sortData () {
      if (this.chartData) {
        ...
        this.chartData.datas.forEach(item2 => {
          ...
          this.healData.push(Number(item2.heal))
          this.deadData.push(Number(item2.dead))
        })
      }
    },
    drawChart () {
      // 基于准备好的dom，初始化echarts实例
      const dom = document.getElementById('daily_growth_chart_china')
